...
public class QueryAdhocManager
{
    ...
    public static string GetQuery(string queryName, string queryPath)
    {
        ...
        {
            var strFilePath = $"{queryPath}/{queryName}";
            ...
            objQuery = new QueryModel
            {
                ...
                queryContent = $"-- {info.FullName}{Environment.NewLine}{Environment.NewLine}{File.ReadAllText(strFilePath, System.Text.Encoding.UTF8)}"
            };
    ...
    public static void LoadAllQuery(string strFilePath, bool startsMonitor = false)
    {
        try
        {
            ...
            foreach (var fl in Directory.GetFiles(strFilePath))
            {
                ...
                if (!ArrQuery().ContainsKey(info.Name))
                {
                    objQuery = new QueryModel
                    {
                        ...
                        filePath = info.FullName,
                        modifiedDate = info.LastWriteTime,
                        queryContent = $"-- {info.FullName}{Environment.NewLine}{Environment.NewLine}{File.ReadAllText(fl, System.Text.Encoding.UTF8)}"
