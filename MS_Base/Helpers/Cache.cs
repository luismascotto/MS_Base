    ...
    public class Cache
    {
        ...
        public ConnectionMultiplexer GetRedisConnection()
        {
            ...
            if (redis == null)
            {
                ...
                string[] arrEndpoint = CacheURL.Split(',');
                server = new List<IServer>();
                foreach (string endPoint in arrEndpoint)
                {
                ...
                sub.Subscribe("CacheBase", (channel, message) => {
                    try
                    {
                        ...
                        {
                            int dataBase = int.Parse(command[1]);
                            foreach (IServer objServer in server)
                            {
        ...
        public bool AddItem(string key, ICache cachedObj, int expiration, int dbID, bool useSeconds = false)
        {
            ...
            if (GetCacheLocation() == (int)CacheLocation.REDIS)
            {
                ...
                db.StringSet(key, JsonConvert.SerializeObject(cachedObj), TimeSpan.FromSeconds(expiration), When.Always, CommandFlags.FireAndForget);
            }
            else //Default Memory
            {
        ...
        public T GetItem<T>(string key, int dbID)
        {
            ...
            if (GetCacheLocation() == (int)CacheLocation.REDIS)
            {
                ...
                try
                {
                    ...
                    if (strObj != null && string.Empty != strObj)
                    {
                        ret = JsonConvert.DeserializeObject<T>(strObj);
                        return (T)ret;
                    }
                }
                catch (Exception)
