...
public class BaseRepository : IDisposable
{
    ...
    {
        List<T> result = new();
        using var command = CreateCommand(dbContext, procName, procParams, procTimeout, CommandType.StoredProcedure);
        OpenDbConnection();
        using var reader = command.ExecuteReader(CommandBehavior.SingleResult);
        while (reader.Read())
        {
            var obj = MapToClass<T>(reader);
            result.Add(obj);
        }
    ...
    protected DbDataReader ExecuteProc(string procName, List<ExecuteProcParam> procParams, int procTimeout = DEFAULT_TIMEOUT)
    {
        using var command = CreateCommand(dbContext, procName, procParams, procTimeout, CommandType.StoredProcedure);
        OpenDbConnection();
        DbDataReader result = command.ExecuteReader(CommandBehavior.CloseConnection);
        return result;
    }
    ...
    {
        using var command = CreateCommand(dbContext, SQL, procParams, procTimeout);
        OpenDbConnection();
        DbDataReader result = command.ExecuteReader(CommandBehavior.CloseConnection);
        return result;
    }
    ...
    private T MapToClass<T>(DbDataReader reader) where T : class
    {
        ...
        var modelProperties = returnedObject.GetType().GetProperties().OrderBy(p => p.MetadataToken).ToList();
        ...
        for (var i = 0; i < reader.FieldCount; i++)
        {
            ...
            var prop = modelProperties.FirstOrDefault(p => string.Equals(p.Name, fieldName, StringComparison.OrdinalIgnoreCase));
            ...
            else
            {
                ...
                if (prop.PropertyType.FullName == "System.String" && (reader.IsDBNull(i)))
                {
                    prop.SetValue(returnedObject, null, null);
                }
                else
    ...
    {
        List<string> ret = new();
        var modelProperties = model.GetProperties().OrderBy(p => p.MetadataToken).ToList();
    ...
    {
        using var command = CreateCommand(dbContext, procName, procParams, procTimeout);
        OpenDbConnection();
        command.ExecuteNonQuery();
        procParams?.ForEach(p =>
            {
    ...
    {
        var dbConnection = dbContext.Database.GetDbConnection();
        if (dbConnection.State == ConnectionState.Open && _dbTransaction != null)
        {
    ...
    protected DbDataReader ExecuteTransactionQueryFromFile(string SQL, List<ExecuteProcParam> procParams, int procTimeout = DEFAULT_TIMEOUT)
    {
        using var command = CreateCommand(dbContext, SQL, procParams, procTimeout);
        command.Transaction = _dbTransaction;
        DbDataReader result = command.ExecuteReader(CommandBehavior.Default);
        return result;
    }

    protected object ExecuteQueryFromFileScalar(string SQL, List<ExecuteProcParam> procParams, int procTimeout = DEFAULT_TIMEOUT)
    {
        using var command = CreateCommand(dbContext, SQL, procParams, procTimeout, CommandType.Text);
        OpenDbConnection();
        object result = command.ExecuteScalar();
        return result;
    }
